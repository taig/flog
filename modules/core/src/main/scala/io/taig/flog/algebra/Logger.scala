...
abstract class Logger[F[_]] {
  ...
  final def debug(message: String): F[Unit] = debug(Scope.Root, message, Json.Null, None)
  final def debug(scope: Scope, message: String): F[Unit] = debug(scope, message, Json.Null, None)
  final def debug(payload: => Json): F[Unit] = debug(Scope.Root, "", payload, None)
  ...
  final def error(message: String): F[Unit] = error(Scope.Root, message, Json.Null, None)
  final def error(scope: Scope, message: String): F[Unit] = error(scope, message, Json.Null, None)
  final def error(payload: => Json): F[Unit] = error(Scope.Root, "", payload, None)
  ...
  final def info(message: String): F[Unit] = info(Scope.Root, message, Json.Null, None)
  final def info(scope: Scope, message: String): F[Unit] = info(scope, message, Json.Null, None)
  final def info(payload: => Json): F[Unit] = info(Scope.Root, "", payload, None)
  final def info(scope: Scope, payload: => Json): F[Unit] = info(scope, "", payload, None)
  ...
  final def warning(message: String): F[Unit] = warning(Scope.Root, message, Json.Null, None)
  final def warning(scope: Scope, message: String): F[Unit] = warning(scope, message, Json.Null, None)
  final def warning(payload: => Json): F[Unit] = warning(Scope.Root, "", payload, None)
